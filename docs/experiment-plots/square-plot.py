...
if __name__ == "__main__":
    ...
    for case in cases:
        ...
        left_encoder_timestamps = [
            (left_encoder[i][0] - left_encoder[0][0]) / 1e9 for i in range(len(left_encoder))
        ]
        left_encoder_distances = [left_encoder[i][1].data for i in range(len(left_encoder))]

        right_encoder_timestamps = [
            (right_encoder[i][0] - right_encoder[0][0]) / 1e9 for i in range(len(right_encoder))
        ]
        right_encoder_distances = [right_encoder[i][1].data for i in range(len(right_encoder))]

        total_encoder_timestamps = [
            (total_encoder[i][0] - total_encoder[0][0]) / 1e9 for i in range(len(total_encoder))
        ]
